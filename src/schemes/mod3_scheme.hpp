...
public:
    ...
    void extend(int p);
    void product(const Mod3Scheme<T> &scheme);
...
private:
    ...
    void addTriplet(int i, int j, int k, const Mod3Vector<T> &u, const Mod3Vector<T> &v, const Mod3Vector<T> &w);
...
template <typename T>
Mod3Scheme<T>::Mod3Scheme() {

}

template <typename T>
Mod3Scheme<T>::Mod3Scheme(const Mod3Scheme &scheme) {
    copy(scheme);
}

template <typename T>
bool Mod3Scheme<T>::initializeNaive(int n1, int n2, int n3) {
    dimension[0] = n1;
    dimension[1] = n2;
    dimension[2] = n3;
    ...
    for (int i = 0; i < n1; i++) {
        for (int j = 0; j < n3; j++) {
            for (int k = 0; k < n2; k++) {
                uvw[0].emplace_back(Mod3Vector<T>(n1 * n2, i * n2 + k));
                uvw[1].emplace_back(Mod3Vector<T>(n2 * n3, k * n3 + j));
                uvw[2].emplace_back(Mod3Vector<T>(n3 * n1, j * n1 + i));
            }
        }
    }
...
bool Mod3Scheme<T>::read(std::istream &is, bool checkCorrectness) {
    ...
    for (int i = 0; i < 3; i++) {
        for (int index = 0; index < rank; index++) {
            Mod3Vector<T> vector(elements[i]);
            is >> vector;
...
template <typename T>
int Mod3Scheme<T>::getComplexity() const {
    int count = 0;
...
template <typename T>
std::string Mod3Scheme<T>::getRing() const {
    return "Z3";
}

template <typename T>
std::string Mod3Scheme<T>::getHash() const {
...
template <typename T>
bool Mod3Scheme<T>::tryPlus(std::mt19937 &generator) {
...
template <typename T>
bool Mod3Scheme<T>::trySplit(std::mt19937 &generator) {
    std::uniform_int_distribution<int> distribution(0, rank - 1);
    int index1, index2, i;
...
template <typename T>
bool Mod3Scheme<T>::tryExpand(std::mt19937 &generator) {
...
template <typename T>
bool Mod3Scheme<T>::trySandwiching(std::mt19937 &generator) {
    ...
    do {
        u.random(0, 2, 1, generator);
    } while (!u.invertible(u1));

    do {
        v.random(0, 2, 1, generator);
    } while (!v.invertible(v1));

    do {
        w.random(0, 2, 1, generator);
    } while (!w.invertible(w1));

    if (!u1.toRing(3) || !v1.toRing(3) || !w1.toRing(3))
        return false;
    ...
    for (int index = 0; index < rank; index++) {
        ...
        for (int i = 0; i < elements[0]; i++)
            uvw[0][index].set(i, mu[i].numerator());

        for (int i = 0; i < elements[1]; i++)
            uvw[1][index].set(i, mv[i].numerator());

        for (int i = 0; i < elements[2]; i++)
            uvw[2][index].set(i, mw[i].numerator());
    }
...
template <typename T>
bool Mod3Scheme<T>::tryProject(std::mt19937 &generator, int minN) {
...
template <typename T>
bool Mod3Scheme<T>::tryExtend(std::mt19937 &generator, int maxN, int maxRank) {
    std::vector<int> indices;
...
template <typename T>
bool Mod3Scheme<T>::tryMerge(const Mod3Scheme<T> &scheme, std::mt19937 &generator, int maxN, int maxRank) {
    std::vector<int> indices;
...
template <typename T>
bool Mod3Scheme<T>::tryProduct(const Mod3Scheme<T> &scheme, int maxN, int maxRank) {
...
template <typename T>
void Mod3Scheme<T>::setSizes(int n1, int n2, int n3) {
...
template <typename T>
void Mod3Scheme<T>::swapSizes(std::mt19937 &generator) {
    int p1, p2;
...
template <typename T>
void Mod3Scheme<T>::swapSizes(int p1, int p2) {
    if (p1 == p2)
        return;
    ...
    for (int index = 0; index < rank; index++) {
        Mod3Vector<T> u(dimensionNew[0] * dimensionNew[1]);
        Mod3Vector<T> v(dimensionNew[1] * dimensionNew[2]);
        Mod3Vector<T> w(dimensionNew[2] * dimensionNew[0]);
...
template <typename T>
void Mod3Scheme<T>::merge(const Mod3Scheme<T> &scheme, int p) {
    int dimensionNew[3];
    int elementsNew[3];
    int d[3];
    ...
    for (int index = 0; index < rank; index++) {
        Mod3Vector<T> u(elementsNew[0]);
        Mod3Vector<T> v(elementsNew[1]);
        Mod3Vector<T> w(elementsNew[2]);
...
template <typename T>
void Mod3Scheme<T>::project(int p, int q) {
    excludeRow(p, q);
...
template <typename T>
void Mod3Scheme<T>::extend(int p) {
    addRow(p);
    addColumn((p + 2) % 3);
    ...
    if (p == 0) {
        for (int i = 0; i < dimension[2]; i++) {
            for (int j = 0; j < dimension[1]; j++) {
                Mod3Vector<T> u((dimension[0] + 1) * dimension[1], dimension[0] * dimension[1] + j);
                Mod3Vector<T> v(dimension[1] * dimension[2], j * dimension[2] + i);
                Mod3Vector<T> w(dimension[2] * (dimension[0] + 1), i * (dimension[0] + 1) + dimension[0]);
                addTriplet(0, 1, 2, u, v, w);
            }
        }
    }
        ...
        for (int i = 0; i < dimension[0]; i++) {
            for (int j = 0; j < dimension[2]; j++) {
                Mod3Vector<T> u(dimension[0] * (dimension[1] + 1), i * (dimension[1] + 1) + dimension[1]);
                Mod3Vector<T> v((dimension[1] + 1) * dimension[2], dimension[1] * dimension[2] + j);
                Mod3Vector<T> w(dimension[2] * dimension[0], j * dimension[0] + i);
                addTriplet(0, 1, 2, u, v, w);
            }
        }
    }
        ...
        for (int i = 0; i < dimension[0]; i++) {
            for (int j = 0; j < dimension[1]; j++) {
                Mod3Vector<T> u(dimension[0] * dimension[1], i * dimension[1] + j);
                Mod3Vector<T> v(dimension[1] * (dimension[2] + 1), j * (dimension[2] + 1) + dimension[2]);
                Mod3Vector<T> w((dimension[2] + 1) * dimension[0], dimension[2] * dimension[0] + i);
                addTriplet(0, 1, 2, u, v, w);
            }
        }
    }
...
template <typename T>
void Mod3Scheme<T>::product(const Mod3Scheme<T> &scheme2) {
    Mod3Scheme<T> scheme1(*this);
    ...
    for (int index1 = 0; index1 < scheme1.rank; index1++) {
        for (int index2 = 0; index2 < scheme2.rank; index2++) {
            for (int p = 0; p < 3; p++) {
                ...
                Mod3Vector<T> vector(elements[p]);
...
template <typename T>
void Mod3Scheme<T>::saveJson(const std::string &path) const {
    std::ofstream f(path);
...
template <typename T>
void Mod3Scheme<T>::saveTxt(const std::string &path) const {
    std::ofstream f(path);
...
template <typename T>
void Mod3Scheme<T>::copy(const Mod3Scheme &scheme) {
    rank = scheme.rank;
...
template <typename T>
bool Mod3Scheme<T>::validate() const {
    for (int i = 0; i < elements[0]; i++)
...
template <typename T>
bool Mod3Scheme<T>::lift(FractionalScheme& lifted, int steps) const {
    if (reconstruct(lifted) && lifted.validate())
        return true;
    ...
    for (int step = 1; step < steps && !reconstructed && lifter.lift(); step++)
...
template <typename T>
void Mod3Scheme<T>::removeZeroes() {
    for (int index = 0; index < rank; index++)
...
template <typename T>
void Mod3Scheme<T>::addTriplet(int i, int j, int k, const Mod3Vector<T> &u, const Mod3Vector<T> &v, const Mod3Vector<T> &w) {
...
template <typename T>
void Mod3Scheme<T>::excludeColumn(int matrix, int column) {
...
template <typename T>
void Mod3Scheme<T>::excludeRow(int matrix, int row) {
    int n1 = dimension[matrix];
    int n2 = dimension[(matrix + 1) % 3];
...
template <typename T>
void Mod3Scheme<T>::addColumn(int matrix) {
    int n1 = dimension[matrix];
    int n2 = dimension[(matrix + 1) % 3];
...
template <typename T>
void Mod3Scheme<T>::addRow(int matrix) {
    int n1 = dimension[matrix];
    int n2 = dimension[(matrix + 1) % 3];
...
template <typename T>
void Mod3Scheme<T>::plus(int i, int j, int k, int index1, int index2, int variant) {
    Mod3Vector<T> a1(uvw[i][index1]);
    Mod3Vector<T> b1(uvw[j][index1]);
    Mod3Vector<T> c1(uvw[k][index1]);

    Mod3Vector<T> a2(uvw[i][index2]);
    Mod3Vector<T> b2(uvw[j][index2]);
    Mod3Vector<T> c2(uvw[k][index2]);

    Mod3Vector<T> aAdd = a1 + a2;
    Mod3Vector<T> bAdd = b1 + b2;
    Mod3Vector<T> cAdd = c1 + c2;

    Mod3Vector<T> aSub = a2 - a1;
    Mod3Vector<T> bSub = b2 - b1;
    Mod3Vector<T> cSub = c2 - c1;
...
template <typename T>
void Mod3Scheme<T>::split(int i, int j, int k, int index1, int index2) {
    Mod3Vector<T> u = uvw[i][index1] - uvw[i][index2];
    Mod3Vector<T> v(uvw[j][index1]);
    Mod3Vector<T> w(uvw[k][index1]);
...
template <typename T>
bool Mod3Scheme<T>::isValidExtension(int p, int maxN, int maxRank) const {
...
template <typename T>
bool Mod3Scheme<T>::isValidProduct(const Mod3Scheme<T> &scheme, int maxN, int maxRank) const {
...
template <typename T>
bool Mod3Scheme<T>::isValidMerge(int p, const Mod3Scheme<T> &scheme, int maxN, int maxRank) const {
    int j = (p + 1) % 3;
    int k = (p + 2) % 3;
...
template <typename T>
bool Mod3Scheme<T>::validateDimensions() const {
...
template <typename T>
bool Mod3Scheme<T>::validateEquation(int i, int j, int k) const {
...
template <typename T>
void Mod3Scheme<T>::saveMatrix(std::ofstream &f, std::string name, const std::vector<Mod3Vector<T>> &vectors) const {
    f << "    \"" << name << "\": [" << std::endl;
